...
def get_img_from_stdin():
    ...
    img = np.load(img_bytes_io)
    return img

def send_array_to_stdout(arr):
    """
    Serialize a NumPy array and send it to stdout.
    
    Parameters:
    arr (numpy.ndarray): The NumPy array to be sent.
    """
    # Serialize the array to a BytesIO object
    arr_bytes_io = io.BytesIO()
    np.save(arr_bytes_io, arr)
    arr_bytes = arr_bytes_io.getvalue()
    
    # Send the size of the array data as a 4-byte header
    sys.stdout.buffer.write(len(arr_bytes).to_bytes(4, byteorder='big'))
    
    # Send the actual array data
    sys.stdout.buffer.write(arr_bytes)
    
    # Flush stdout to ensure the data is sent
    sys.stdout.flush()
if __name__ == '__main__':
    warnings.filterwarnings("ignore")
    warnings.filterwarnings("ignore", message="You are using a SMPL model, with only 10 shape coefficients.")
    ...
    # Preprocess input image and generate predictions
        ...
        img = get_img_from_stdin()
        norm_img = torch.from_numpy(get_img_from_stdin())
        with torch.no_grad():
            ...
            # print("FPS:", 1 / elapsed_time, "FPS")
            # print("pred_betas =", pred_betas)
            pred_rotmat[0, 0] = identity_matrix
            pred_rotmat = pred_rotmat.squeeze(0).numpy()
            send_array_to_stdout(pred_rotmat)
            ...
            if iter < num_saved_frames:
                cv2.imwrite(f'examples/frame_{iter}.png', 255 * img_bgr)
                iter += 1
